    ...
    def login(cls, email: str, password: str) -> tuple[LoginResponse, str]:
        ...
        with MongoClient(MONGO_CS) as c:
            ...
            if not bcrypt.checkpw(
                password.encode(), user.password_hash.get_secret_value().encode()
            ):
        ...
        with MongoClient(MONGO_CS) as c:
            with c.start_session() as s:
                with s.start_transaction() as t:
                    ...
                    id = c[DB].accounts.insert_one(user.model_dump()).inserted_id
                    ...
                    if notify:
                        if not cls.send_activation_email(user.email, activation_key):
                ...
                return cls.login(user.email, user.password)
